...
class Credential extends Endpoint {
  ...
  async getList({alias = '', initiator = '', invitation_key = '', my_did = '', state = '', their_did = '', their_role = ''}){
    this.axios = axios.get('/credential', {
      params: {
        alias,
        initiator,
        invitation_key,
  ...
  async mimeTypes(credential_id) {
    let response = this.axios.get(`/credential/mime-types/${credential_id}`);
    return {
  ...
  async revoked(credential_id, { from, to }) {
    let response = this.axios.get(`/credential/revoked/${credential_id}`, {
      params: {
        from,
        to
      }
  ...
  // @param {String} credential_id [(*required) Connection identifier]
  async mimeTypes(credential_id) {
    let response = this.axios.get(`/credential/w3c/${credential_id}`);
    return {
      status: response.status,
      result: response.data
    }
  ...
  // @param {String} credential_id [(*required) Connection identifier]
  async mimeTypes(credential_id) {
    let response = this.axios.delete(`/credential/w3c/${credential_id}`);
    return {
      status: response.status,
      result: response.data
    }
  ...
  // @param {String} credential_id [(*required) Connection identifier]
  async mimeTypes(credential_id) {
    let response = this.axios.get(`/credential/${credential_id}`);
    return {
      status: response.status,
      result: response.data
    }
  ...
  async deleteOne(credential_id) {
    let response = this.axios.delete(`/credential/${credential_id}`);
    return {
      status: response.status,
      result: response.data
    }
