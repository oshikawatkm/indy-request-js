...
class PresentProofV2 extends Endpoint {
  ...
  async records({ connection_id, role, state, thread_id }) {
    let response = await this.axios.get('/present-proof-2.0/records', { 
      connection_id, 
      role, 
      state, 
      thread_id
  ...
  async recordsVerifyPresentation(pres_ex_id) {
    let response = await this.axios.post(`/present-proof-2.0/records/${pres_ex_id}/verify-presentation`, body)
    return {
      status:   response.status,
      results:  response.data
    }
